  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        {user && user.currentLevel && user.targetLevel && (
          <Card className="lg:col-span-1 border-none shadow-xl shadow-slate-200/50 bg-gradient-to-br from-primary to-indigo-700 text-primary-foreground overflow-hidden relative">
        ...
        <div className="lg:col-span-2 grid gap-4 grid-cols-2">
          <Card className="border-none shadow-md shadow-slate-200/60 transition-all hover:shadow-lg">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="border-none shadow-lg shadow-slate-200/60 overflow-hidden">
          ...
          <CardContent className="grid gap-3 pt-6">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-3 text-slate-600"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
              맞춤 설정 (서베이) 수정
            </Button>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-8">
            <div className="space-y-8">
              {(() => {
                ...
                const progressPercent = Math.round(((stats?.inProgressQuestions || 0) / total) * 100);
                ...
                return (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-2">
                      ...
                      <div className="text-center p-3 rounded-xl bg-blue-50 border border-blue-100/50">
                        <div className="text-xl font-bold text-blue-700">{stats?.inProgressQuestions || 0}</div>
                        <div className="text-[10px] text-blue-600 font-bold uppercase tracking-wider">진행</div>
                      </div>
                      <div className="text-center p-3 rounded-xl bg-slate-50 border border-slate-100/50">
                        <div className="text-xl font-bold text-slate-700">{stats?.notAttemptedQuestions || 0}</div>
                        <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">미시도</div>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="divide-y divide-slate-100">
            {recentFeedbacks.length > 0 ? (
              recentFeedbacks.map((feedback) => (
                <div key={feedback.id} className="group flex items-center gap-6 p-6 transition-all hover:bg-slate-50 cursor-pointer" onClick={() => router.push(`/history/${feedback.id}`)}>
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white shadow-sm border border-slate-100 group-hover:border-primary/20 group-hover:shadow-md transition-all">
                  ...
                  <div className="text-right shrink-0">
                    <div className="text-lg font-black text-slate-700">{feedback.totalScore}<span className="text-xs font-normal text-slate-400 ml-0.5">/ 100</span></div>
                    <div className="mt-1 flex gap-1 justify-end">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <div key={s} className={`h-1 w-3 rounded-full ${s <= (feedback.totalScore / 20) ? 'bg-primary' : 'bg-slate-200'}`} />
                      ))}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4 border border-slate-100">
                  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-slate-300"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>
                </div>
                <h3 className="text-lg font-semibold text-slate-600">아직 연습 기록이 없네요</h3>
                <p className="text-slate-400 mt-1 max-w-xs">연습을 시작하고 AI의 정밀한 피드백을 받아보세요.</p>
